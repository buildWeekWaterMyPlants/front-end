...
function Plant(props) {
  ...
  const calculateMsLeft = () =>
    msPerDay * h2oFrequency - (Date.now() - lastWaterTime);

  const [timeLeft, setTimeLeft] = useState(calculateMsLeft());

  const calculateSecondsLeft = () =>
    Math.floor((timeLeft % msPerMinute) / 1000);
  const calculateMinutesLeft = () =>
    Math.floor((timeLeft % msPerHour) / msPerMinute);
  const calculateHoursLeft = () =>
    Math.floor((timeLeft % msPerDay) / msPerHour);
    ...
    <div className="border rounded-lg flex w-1/4 m-4 shadow-lg  bg-green-300">
      <div className="flex flex-col p-6">
        ...
        <h6 className="text-sm text-left animate-pulse">
          {daysLeft} days, {hoursLeft} hours, {minutesLeft} minutues,{" "}
          {secondsLeft} seconds.
        </h6>
        <button
